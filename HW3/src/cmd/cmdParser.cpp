...
CmdParser::parseCmd(string& option)
{
   ...
   string cmd;
   size_t OptPos = myStrGetTok(str, cmd);
   option = OptPos==string::npos? "" : str.substr(OptPos);
   return getCmd(cmd);
}
...
CmdParser::listCmd(const string& str)
{
   ...
   if (tok.size() == 0){
      ...
      for (CmdMap::iterator i=_cmdMap.begin(); i!=_cmdMap.end(); ++i){
         string cmdName = i->first;
         cmdName += (i->second)->getOptCmd();
         cout << setw(12) << left << cmdName;
         ++count;
         if (count%5 == 0) cout << endl;
      }
      reprintCmd(); return;
   }
   //case 2: Partially Match
   vector<string> MatchCmd;
   for (CmdMap::iterator i=_cmdMap.begin(); i!=_cmdMap.end(); ++i){
      string cmdName = i->first;
      cmdName += (i->second)->getOptCmd();
      if (tok.size()>cmdName.size()) continue;
      if (myStrNCmp(cmdName, tok, tok.size()) == 0 )    //Must totally match.
         MatchCmd.push_back(cmdName);
   }
      //Multiple matches:
   if (MatchCmd.size() > 1){
      cout << endl;
      for (size_t i=0; i<MatchCmd.size(); i++){
         cout << setw(12) << left << MatchCmd[i];
         if (i%5==4 && i!=MatchCmd.size()-1) cout << endl;
      }
      reprintCmd(); return;
   }
      //Single match:
   else if (MatchCmd.size() == 1){
      string seg(MatchCmd[0].begin()+tok.size(), MatchCmd[0].end());
      if (seg.size() == 0){
         cout << endl;
         getCmd(tok)->usage(cout);
         reprintCmd();
      }
      else{
...
CmdParser::getCmd(string cmd)
{
   ...
   for (CmdMap::iterator i=_cmdMap.begin(); i!=_cmdMap.end(); ++i){
      string s=i->first;
      int mand = s.size();
      s += (i->second)->getOptCmd();
      if (myStrNCmp(s,cmd,mand) == 0)
         e=i->second;
   }
   return e;
}
