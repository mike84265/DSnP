...
static char checkArg(const string& str)
{
   if (myStrNCmp("-Row",str,2)==0)
      return 'R';
   else if (myStrNCmp("-Column",str,2)==0)
      return 'C';
   else
      return 'E';
}
...
initDbCmd()
{
   ...
   if (!(cmdMgr->regCmd("DBAPpend", 4, new DBAppendCmd) &&
      ...
      cerr << "Register \"db\" commands fails... exiting" << endl;
      return false;
   }
   return true;
}
...
DBAppendCmd::exec(const string& option)
{
   ...
   if (!dbtbl){
      cerr << "Table is not yet created!!" << endl;
      return CMD_EXEC_ERROR;
   }
   char arg = checkArg(tokens[0]);
   DBRow NewRow;
   vector<int> NewCol;
   switch(arg){
     case 'R':
      if (tokens.size() < dbtbl.nCols()+1){
         for (size_t i=tokens.size(); i<=dbtbl.nCols(); i++)
            tokens.push_back("-");
      }
      ...
      assert(NewRow.size() == dbtbl.nCols() );
      if (NewRow.size() != 0) dbtbl.addRow(NewRow);
      break;
    case 'C':
      if (tokens.size() < dbtbl.nRows()+1){
         for (size_t i=tokens.size(); i<=dbtbl.nRows(); i++)
            tokens.push_back("-");
      }
      for (size_t i=1; i<=dbtbl.nRows(); i++){
         if (tokens[i].compare("-") == 0)
            NewCol.push_back(INT_MAX);
         else{
            int num;
            if (!myStr2Int(tokens[i],num)){
               cerr << tokens[i] << "is not a number!!" << endl;
               return CMD_EXEC_ERROR;
            }
            NewCol.push_back(num);
         }
      }
      assert(NewCol.size() == dbtbl.nRows());
      if (NewCol.size() != 0) dbtbl.addCol(NewCol);
      break;
    case 'E':
    default:
...
{  
   ...
   if (options.size() == 0) return errorOption(CMD_OPT_MISSING,"DBPrint");
   if (myStrNCmp("-Row",options[0],2) == 0){
      if (options.size()<2 || options[1].size()==0 )
      ...
      if (options.size()>1) 
         return errorOption(CMD_OPT_EXTRA,options[1]);
      else cout << dbtbl;
   }  //End of "-Table" option
      ...
      if (!checkColIdx(options[1],col))
         return CMD_EXEC_ERROR;
      DBTable::printData(cout, dbtbl[row][col], true);
   }  //End of "(int Row) (int Column)" option
   else return errorOption(CMD_OPT_ILLEGAL,option);
   return CMD_EXEC_DONE;
}
...
DBReadCmd::exec(const string& option)
{
   ...
   string fileName;
   // "-Replace" can be at either options[0] or options[1].
