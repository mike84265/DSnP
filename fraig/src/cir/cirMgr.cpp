...
CirMgr::writeAag(ostream& outfile) const
{
   ...
   for (size_t i=0;i<_dfsList.size();++i){
      ...
      outfile << g->getID()*2;
      for (size_t j=0;j<2;j++){
         unsigned ID = toPtr(g->getFanin(j))->getID() * 2;
         if (g->getFanin(j) & INV)
            ++ID;
         outfile << " " << ID;
      }
...
CirMgr::buildDfsList()
{
   ...
   for (unsigned i=0;i<_PONum;++i)
      pushFanin(_gateList[ _idList[_PO_begin+i] ]);
}

void
CirMgr::pushFanin(CirGate* g)
{
   if (g->refMatch() || g->getTypeStr() == "UNDEF") return;
   if (g->getTypeStr() == "AIG"){
      AIG* t = dynamic_cast<AIG*>(g);
      for (int i=0;i<2;++i)
         pushFanin(toPtr(t->getFanin(i)));
   } else if (g->getTypeStr() == "PO") {
      PO* t = dynamic_cast<PO*>(g);
      pushFanin(toPtr(t->getFanin()));
   } 
